...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        x = []
        for i in w:
            embedding = self.embeddings[[i]].view(1, -1)
            x.append(embedding)

        # Stack all the embeddings to create the final tensor
        x = torch.cat(x, dim=0)
        return x
    ...
    def forward(self, w):
        ...
        xW = x @ self.embed_to_hidden_weight
        b1 = self.embed_to_hidden_bias
        h = F.relu(xW + b1)

        hU = self.dropout(h) @ self.hidden_to_logits_weight
