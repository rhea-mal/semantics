...
def negSamplingLossAndGradient(
    ...
    z = sigmoid(np.dot(u_o, centerWordVec)) # reuse this calculation

    loss = -np.log(z)
    gradCenterVec = (z - 1) * u_o
    gradOutsideVecs[outsideWordIdx] = (z - 1) * centerWordVec

    for negSampleWordIdx in negSampleWordIndices:
        z = sigmoid(-np.dot(outsideVectors[negSampleWordIdx], centerWordVec))
        loss -= np.log(z)
        gradCenterVec -= (z - 1) * outsideVectors[negSampleWordIdx]
        gradOutsideVecs[negSampleWordIdx] -= (z - 1) * centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    # for each outside word in context window
